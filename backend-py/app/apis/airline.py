...
class AirlineStats(Resource):
    ...
    def get(self, airline_id):
        try:
            cache_key = f'airline_stats:{airline_id}'
            cached_stats = redis_client.get(cache_key)
            if cached_stats:
                return json.loads(cached_stats), 200
            # Fallback if cache missed
            stats = calculate_airline_stats(airline_id)
            redis_client.set(cache_key, json.dumps(stats), ex=3600)  # expire in 1 hour as fallback
            return stats, 200
        except Exception as e:
            import traceback
            traceback.print_exc()
